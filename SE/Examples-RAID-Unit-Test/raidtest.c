...
{    // 1a
	...
	assert(memcmp(&testLBA1[LBAidx], &testLBA4[LBAidx], SECTOR_SIZE) == 0);
	...
	modifyBuffer(PTR_CAST &testLBA1[0]);	
	assert(memcmp(&testLBA1[LBAidx], &testLBA4[LBAidx], SECTOR_SIZE) != 0);

	// Now rebuild LBA into test to verify
        rebuildLBA(PTR_CAST &testLBA2[0],
	           PTR_CAST &testLBA3[0],
	           PTR_CAST &testLBA4[0],
	           PTR_CAST &testPLBA[0],
	           PTR_CAST &testLBA1[0]);	// This si the one I'm modifying
   assert(memcmp(&testLBA1[LBAidx], &testLBA4[LBAidx], SECTOR_SIZE) == 0);

	//// fix data 2
   assert(memcmp(&testLBA2[LBAidx], &testLBA4[LBAidx], SECTOR_SIZE) == 0);
   xorLBA(PTR_CAST &testLBA1[0],
          PTR_CAST &testLBA2[0],
          PTR_CAST &testLBA3[0],
          PTR_CAST &testLBA4[0],
   ...
   modifyBuffer(PTR_CAST &testLBA2[0]);
   assert(memcmp(&testLBA2[LBAidx], &testLBA4[LBAidx], SECTOR_SIZE) != 0);
   ...
   rebuildLBA(PTR_CAST &testLBA1[0],
              ...
              PTR_CAST &testLBA4[0],
              PTR_CAST &testPLBA[0],
              PTR_CAST &testLBA2[0]);    // This si the one I'm modifying
   assert(memcmp(&testLBA2[LBAidx],&testLBA4[LBAidx], SECTOR_SIZE) == 0);
    
//// fix data 3
          
      assert(memcmp(&testLBA3[LBAidx], &testLBA4[LBAidx], SECTOR_SIZE) == 0);
      xorLBA(PTR_CAST &testLBA1[0],
             PTR_CAST &testLBA2[0],
             PTR_CAST &testLBA3[0],
             PTR_CAST &testLBA4[0],
      ...
      modifyBuffer(PTR_CAST &testLBA3[0]);
      assert(memcmp(&testLBA3[LBAidx], &testLBA4[LBAidx], SECTOR_SIZE) != 0);
      ...
      rebuildLBA(PTR_CAST &testLBA1[0],
                 ...
                 PTR_CAST &testLBA4[0],
                 PTR_CAST &testPLBA[0],
                 PTR_CAST &testLBA3[0]);    // This si the one I'm modifying
      assert(memcmp(&testLBA3[LBAidx],&testLBA4[LBAidx], SECTOR_SIZE) == 0);
             
    ////// fix data 4
         assert(memcmp(&testLBA4[LBAidx], &testLBA1[LBAidx], SECTOR_SIZE) == 0);
         xorLBA(PTR_CAST &testLBA1[0],
                PTR_CAST &testLBA2[0],
                PTR_CAST &testLBA3[0],
                PTR_CAST &testLBA4[0],
         ...
         modifyBuffer(PTR_CAST &testLBA4[0]);
         assert(memcmp(&testLBA4[LBAidx], &testLBA1[LBAidx], SECTOR_SIZE) != 0);
         ...
         rebuildLBA(PTR_CAST &testLBA1[0],
                    ...
                    PTR_CAST &testLBA3[0],
                    PTR_CAST &testPLBA[0],
                    PTR_CAST &testLBA4[0]);    // This si the one I'm modifying
         assert(memcmp(&testLBA3[LBAidx],&testLBA4[LBAidx], SECTOR_SIZE) == 0);   
}

        printf("LBA 2 =");
        dumpBuffer((char *)&testLBA2[0]);
        getchar();

        assert(memcmp(testLBA2, testLBA4, SECTOR_SIZE) ==0);
